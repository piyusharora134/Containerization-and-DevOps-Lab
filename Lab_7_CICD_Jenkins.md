...
A custom **Permanent Agent** node (`Piyush node macos`) was also configured in Jenkins under `Manage Jenkins → Nodes → New Node` to demonstrate multi-node agent setup.
...
> Creating the admin user `PiyushArora` during Jenkins initial configuration.
...
> Jenkins user account page for `PiyushArora` showing profile, credentials, and email settings via the localtunnel public URL.

---

### Screenshot 7 — Adding New Node (Agent)
![Jenkins New Node Creation]![alt text](image-7.png)
> Creating a new Permanent Agent node named `Piyush node` in Jenkins under `Manage Jenkins → Nodes → New Node`.
...
> Jenkins Nodes page showing the `Built-In Node` (Linux aarch64) and the newly added `Piyush node macos` agent.
...
> Jenkins displaying the JNLP agent connection commands (Unix/Windows) with a secret token to connect `Piyush node macos` as a remote agent.
